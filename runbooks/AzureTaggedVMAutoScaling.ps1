...
Function Start-VMAutoScaling{ 
        ...
        foreach ($VM in $VMs){ 
            ...
            if($VMName -notin $VMexceptions){ 
                $VMDetail = Get-AzureRmVM -ResourceGroupName $resourceGroupName -Name $VMName 
                $ScaleSize = $VMDetail.Tags[$scaleTagSwitch] 
                $VMSize = $VMDetail.HardwareProfile.VmSize 
                if(($VMSize -ne $ScaleSize) -and ($ScaleSize)){ 
                    Write-Output "Resource Group: $resourceGroupName", ("VM Name: " + $VMName), "Current VM Size: $VMSize", "$scaleTagSwitch : $ScaleSize"  
                    ...
                    if($VMStatus.Statuses[1].DisplayStatus -eq "VM running"){ 
                        ...
                        Stop-AzureRmVM -ResourceGroupName $resourceGroupName -Name $VMName -Force | Out-Null 
                    } 
